...
def scanner():
    ...
    try:
        line(0,0, (height-height*0.12)*cos(radians(Angle.index(0))), -(height-height*0.12)*sin(radians(Angle.index(0))))
        Angle.deleteFirst()
        #print(Angle)
...
def labels():
    ...
    try:
        text("Angle: " + str(Angle.index(0)) +" \xb0", width-width*0.48, height-height*0.0377)
    except AttributeError:
